...
While a starshade has not yet been flown in space, in subscale demonstrations a starshade suppresses starlight by a factor of $10^{-10}$ or more, across a broad range of wavelengths needed to obtain measurements of water, oxygen, methane among other biogases [@mennesson]. Beyond suppressing starlight lie specific questions about the properties and configurations of exoplanetary systems that can be imaged, and which instrument designs are most suitable. `PyStarshade` is a Python toolbox to efficiently perform optical simulations of starshades with exoplanetary scenes to assess the utility of starshades in future direct-imaging missions.
