...
def pupil_to_ccd(
    wl, focal_length_lens, pupil_field, pupil_mask, dt, dp, N_t, N_pix
):
    """
...
def source_field_to_pupil(
    ss_mask_fname, wl, dist_ss_t, N_x=6401, N_t=1001, dx=0.01, dt=0.03, chunk=1
):
    """
    ...
    if chunk and not ss_mask_fname.endswith(".dat"):
        raise ValueError("starshade mask must be a memmap file")
    N_s = 11
    ds = 0.04 * au_to_meter
    ...
    if chunk:
        field_incident_telescope_compl, dt = (
            fresnel.nchunk_zoom_fresnel_single_fft(
                ss_mask_fname, N_t, N_chunk=4
            )
        )
    else:
        starshade_qu = np.load(ss_mask_fname)
        starshade = qu_mask_to_full(starshade_qu)
        field_incident_telescope_compl, dt = fresnel.zoom_fresnel_single_fft(
            starshade, field_after_ss, N_t
        )
...
def chunk_source_field_to_pupil(
    source_field,
    wl,
    dist_xo_ss,
    dist_ss_t,
    ...
    for chunk in range(4):
        ...
        field_after_ss = np.load(ss_mask_fname)["grey_mask"].astype(
            np.complex128
        )
        ...
        fresnel = FresnelSingle(dx, dt, N_x, dist_ss_t, wl)
        field_incident_telescope_compl_quad, dt = (
            fresnel.one_chunk_zoom_fresnel_single_fft(
                self, field, N_out, chunk=0
            )(field_after_ss, N_t, chunk=chunk)
        )
...
def source_field_to_ccd(
    ...
    N_xt, N_yt = N_in_2d(
        pupil_mask
    )  # Number of non-zero samples in telescope pupil

    field_aperture_freesp = bluestein_pad(
        field_free_prop * pupil_mask, N_xt, N_pix
    )
    field_aperture_ss = bluestein_pad(test_new * pupil_mask, N_xt, N_pix)

    fraunhofer = Fraunhofer(dt, dp, N_xt, focal_length_lens, wl)
    out_field_free_space, dp = fraunhofer.zoom_fraunhofer(
        field_aperture_freesp, N_pix
    )
    out_field_compl_ss, dp = fraunhofer.zoom_fraunhofer(
        field_aperture_ss, N_pix
    )
...
def point_source_to_ccd(
    ...
    N_X = (
        N_x + N_t - 1
    )  # The number of input samples required to perform Fresnel propagation with a Bluestein FFT
    ...
    ss_complement = 1 - eval_hypergauss(
        N_X, dx
    )  # Complement of starshade aperture
