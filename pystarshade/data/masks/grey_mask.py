...
mask = np.loadtxt(
    "starshade_masks/wfirst_mask_002m"
)  # generated with 0.001m grid
print(np.shape(mask))

up_fac = 5
grey_mask = convolve(mask, np.ones((up_fac, up_fac)))[
    ::up_fac, ::up_fac
].astype(np.float32)
...
np.savetxt("starshade_masks/grey_wfirst_mask_01m", full_mask)
