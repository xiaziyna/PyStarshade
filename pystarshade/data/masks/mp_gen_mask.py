...
(r0, r1, x_vals_ss, y_vals_ss) = pickle.load(
    open("starshade_edge_files/wfirst_locus.p", "rb")
)
...
up_sample = 5
dx = dy = 0.1 / up_sample
Nx = Ny = int(((r1 + 0.2) * up_sample) / 0.1)
...
for i in range(num_processes_dim):
    for j in range(num_processes_dim):
        ...
        if i == num_processes_dim - 1:
            l_x += l_mask_ghost
        if j == num_processes_dim - 1:
            l_y += l_mask_ghost
        l_mem = l_x * l_y
        mask_.append(mp.RawArray(ctypes.c_bool, l_mem))
        p = mp.Process(
            target=eval_chunk,
            args=(semaphore, i, j, l_x, l_y, mask_[i * num_processes_dim + j]),
        )
        processes.append(p)
...
for i in range(num_processes_dim):
    for j in range(num_processes_dim):
        ...
        if j == num_processes_dim - 1:
            l_y += l_mask_ghost
        l_mem = l_x * l_y
        mask_chunk = np.reshape(
            np.frombuffer(mask_[i * num_processes_dim + j], dtype=bool), -1
        )
        mask_chunk = mask_chunk.reshape((l_x, l_y))
        mask[
            i * l_mask_chunk : i * l_mask_chunk + l_x,
            j * l_mask_chunk : j * l_mask_chunk + l_y,
        ] = mask_chunk

mask = mask.astype(np.float64)
grey_mask = convolve(mask, np.ones((up_sample, up_sample)))[
    ::up_sample, ::up_sample
].astype(np.float64)
grey_mask /= up_sample**2
np.savez_compressed(
    "starshade_masks/grey_hwo_24_mask_" + final_dx_str + "m_qu.npz",
    grey_mask=grey_mask,
)
