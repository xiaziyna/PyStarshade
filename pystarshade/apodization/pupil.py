...
def custom_hex(normalized=True):
    ...
    segment_flat_to_flat = (
        pupil_diameter - (2 * num_rings + 1) * gap_size
    ) / (2 * num_rings + 1)
    return make_hexagonal_segmented_aperture(
        num_rings, segment_flat_to_flat, gap_size
    )
...
def make_pupil(N_t, pupil_type="circ"):
    ...
    pupil = evaluate_supersampled(
        call_aperture_function(pupil_type, normalized=True), pupil_grid, 8
    )
    pupil = np.reshape(pupil, (N_t, N_t))
    np.savez_compressed(
        "data/pupils/" + pupil_type + "_" + str(int(N_t)) + ".npz", pupil=pupil
    )
