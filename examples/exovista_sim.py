...
extended_field[
    :,
    half_N - half_in_N : half_N + half_in_N + bit_arr,
    half_N - half_in_N : half_N + half_in_N + bit_arr,
] = bg_scene

n_planet = 0
for i in range(1, scene.nplanets):
    if np.all(planet_xy[i] - N_dust_half + half_N <= N_s - 1) and np.all(
        planet_xy[i] - N_dust_half + half_N >= 0
    ):
        extended_field[
            :,
            planet_xy[i, 1] - N_dust_half + half_N,
            planet_xy[i, 0] - N_dust_half + half_N,
        ] += planet_spectra[i]
        n_planet += 1
...
print(
    "Plan/Star brightness: ", (planet_spectra[1:, w_i] / (star_spectra[w_i]))
)
print(
    "Semi major axis[au]: ",
    ds_au
    * np.hypot(planet_xy[1:, 0] - N_dust_half, planet_xy[1:, 1] - N_dust_half),
)
...
focal_intensity = hwo_starshade.gen_scene(
    pupil_type, source_field.astype(np.float32), 500e-9, pupil_symmetry=True
)
...
save_path = data_file_path(
    "exo_scene_" + drm + "_" + pupil_type + ".npz", "out"
)
print(save_path)
