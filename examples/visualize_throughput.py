...
data_dir = os.path.abspath(
    os.path.join(script_dir, "..", "pystarshade", "data")
)
...
through_fname_on = os.path.join(
    data_dir, "psf", "hwo_throughput_hwopupil_onaxis_001m.npz"
)
through_fname_off = os.path.join(
    data_dir, "psf", "hwo_throughput_hwopupil_offaxis_001m.npz"
)
...
total_throughput_on = data_throughput_on["total_throughput"][
    :, N // 2, N // 2 :
]
core_throughput_on = data_throughput_on["core_throughput"][:, N // 2, N // 2 :]

data_throughput_off = np.load(through_fname_off)
total_throughput_off = data_throughput_off["total_throughput"][
    :, N // 2, N // 2 :
]
core_throughput_off = data_throughput_off["core_throughput"][
    :, N // 2, N // 2 :
]
...
for i in range(len(wl_)):
    ...
    ax1.plot(
        np.arange(len(core_throughput_off[i])) * 2,
        core_throughput_off[i],
        color=colors[i + 4],
        linewidth=1.0,
        label="off-axis pupil",
    )
    plt1 = ax1.plot(
        np.arange(len(core_throughput_on[i])) * 2,
        core_throughput_on[i],
        color=colors2[i + 4],
        linewidth=1.0,
        label="on-axis pupil",
    )
    ...
    loc_max_off = get_nearest(
        max_val_on, core_throughput_off[i][: loc_max_on + 1]
    )

    if i == len(wl_) - 1:
        plt.axvline(
            np.arange(len(core_throughput_on[-1]))[loc_max_on] * 2,
            linestyle="dashed",
            color=colors2[i + 4],
        )
        plt.axvline(
            np.arange(len(core_throughput_on[-1]))[loc_max_off] * 2,
            linestyle="dashed",
            color=colors[i + 4],
        )
...
cbar_ax = fig.add_axes([0.02, 0.12, 0.01, 0.75])
cbar_ax2 = fig.add_axes([0.06, 0.12, 0.01, 0.75])
fig.colorbar(
    mappable=cmappable, cax=cbar_ax, boundaries=[500, 600, 700, 800, 900, 1000]
)
fig.colorbar(
    mappable=cmappable2,
    cax=cbar_ax2,
    boundaries=[500, 600, 700, 800, 900, 1000],
)
fig.text(
    0.005,
    0.5,
...
fig.text(
    0.055, 0.9, r"On-axis", va="center", rotation="horizontal", fontsize=14
)
fig.text(
    0.005, 0.9, r"Off-axis", va="center", rotation="horizontal", fontsize=14
)
fig.text(
    0.005, 0.93, r"Pupil:", va="center", rotation="horizontal", fontsize=14
)
