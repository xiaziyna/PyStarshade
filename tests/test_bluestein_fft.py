...
from pystarshade.diffraction.bluestein_fft import (
    zoom_fft_2d,
    zoom_fft_2d_mod,
    zoom_fft_quad_out_mod,
)
from pystarshade.diffraction.util import N_in_2d, bluestein_pad, trunc_2d
...
def sim_data_square(N_x, N_X):
    x = np.zeros((N_X, N_X))
    x[
        N_X // 2 - N_x // 2 : N_X // 2 + N_x // 2 + 1,
        N_X // 2 - N_x // 2 : N_X // 2 + N_x // 2 + 1,
    ] = np.ones((N_x, N_x))
    x[N_X // 2 - 2 : N_X // 2, N_X // 2 + 1] *= random.uniform(0.2, 10)
    return x
...
def test_quad_out(N_x, N_X, N_out, Z_pad, N_chirp, x):
    ...
    real_ft_quad0 = real_ft_x[
        N_X // 2 - N_out : N_X // 2, N_X // 2 - N_out : N_X // 2
    ]
    real_ft_quad1 = real_ft_x[
        N_X // 2 - N_out : N_X // 2, N_X // 2 : N_X // 2 + N_out
    ]
    real_ft_quad2 = real_ft_x[
        N_X // 2 : N_X // 2 + N_out, N_X // 2 - N_out : N_X // 2
    ]
    real_ft_quad3 = real_ft_x[
        N_X // 2 : N_X // 2 + N_out, N_X // 2 : N_X // 2 + N_out
    ]
...
def test_chunked_bluestein_fft(N_x, N_X, N_out, Z_pad, N_chirp, x):
    ...
    x_memmap = np.memmap(
        "mm_data.dat", dtype=np.complex128, mode="w+", shape=(N_x, N_x)
    )
    x_memmap[:] = x_trunc[:]
    x_memmap.flush()
...
def test_chunked_bluestein_fft_verbose(N_x, N_X, N_out, Z_pad, N_chirp, x):
    ...
    sec_N_x = (
        N_x // 2 + 1
    )  # this is the (max) size of non-zero portion of segment, same for all 4 chunks
    sec_N_x += (
        1 - sec_N_x % 2
    )  # want this to be an odd number as it makes it easier to calculate shifts and center
    ...
    out_fac = np.exp(
        ...
        * (1j * 2 * np.pi * (phase_shift - shift_bit) * (1 / (N_X)))
    )

    x_test = np.zeros((N_X, N_X))
    x_test[: N_X // 2 + bit_x, : N_X // 2 + bit_x] = x_1

    real_ft_x_1 = np.fft.fftshift(np.fft.fft2(x_test))[
        N_X // 2 - N_out // 2 : N_X // 2 + N_out // 2 + 1,
        N_X // 2 - N_out // 2 : N_X // 2 + N_out // 2 + 1,
    ]
    ...
    out_fac_2 = np.exp(
        np.arange(-(N_out // 2), (N_out // 2) + 1)
        * (1j * 2 * np.pi * -(phase_shift + 1) * (1 / (N_X)))
    )

    x_test = np.zeros((N_X, N_X))
    x_test[: N_X // 2 + bit_x, N_X // 2 + bit_x :] = x_2

    real_ft_x_2 = np.fft.fftshift(np.fft.fft2(x_test))[
        N_X // 2 - N_out // 2 : N_X // 2 + N_out // 2 + 1,
        N_X // 2 - N_out // 2 : N_X // 2 + N_out // 2 + 1,
    ]
    ...
    real_ft_x_3 = np.fft.fftshift(np.fft.fft2(x_test))[
        N_X // 2 - N_out // 2 : N_X // 2 + N_out // 2 + 1,
        N_X // 2 - N_out // 2 : N_X // 2 + N_out // 2 + 1,
    ]
    ...
    real_ft_x_4 = np.fft.fftshift(np.fft.fft2(x_test))[
        N_X // 2 - N_out // 2 : N_X // 2 + N_out // 2 + 1,
        N_X // 2 - N_out // 2 : N_X // 2 + N_out // 2 + 1,
    ]
    ...
    assert np.allclose(
        real_ft_x_1 + real_ft_x_2 + real_ft_x_3 + real_ft_x_4, test_chunked_bs
    )
