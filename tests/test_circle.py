...
dist_ss_t = 63942090.0  # distance for hypergaussian mask
N_t = 257
N_x = 4001
dt = R_lens / (N_t // 2)
dx = R_ss / (N_x // 2)

test_data = [(wl, dist_xo_ss, dist_ss_t, R_ss, N_x, N_t, dx, dt)]


def calculate_circle_solution(
    wl, dist_xo_ss, dist_ss_t, R_ss, N_t=N_t, dt=dt, is_opaque=True
):
...
def get_field(uu, vv, ss, sv_inds, kk, zz, z0, is_opaque=True, is_shadow=True):
    ...
    EE = np.exp(1j * uu / 2.0) * (AA + 1j * BB * [1.0, -1.0][int(is_shadow)])

    # Add illuminated beam
    if not is_shadow:
        EE += np.exp(-1j * vv[sv_inds] ** 2.0 / (2.0 * uu))

    # Add final plane QPF phase terms
    EE *= np.exp(1j * kk * (ss[sv_inds] ** 2.0 / (2.0 * zz) + zz))
...
def lommels_V(u, v, nt=10):
    ...
    VV_1 = 0.0
    for m in range(nt):
        VV_0 += (-1.0) ** m * (v / u) ** (0 + 2.0 * m) * jn(0 + 2 * m, v)
        VV_1 += (-1.0) ** m * (v / u) ** (1 + 2.0 * m) * jn(1 + 2 * m, v)
    return VV_0, VV_1
...
def lommels_U(u, v, nt=10):
    ...
    UU_2 = 0.0
    for m in range(nt):
        UU_1 += (-1.0) ** m * (u / v) ** (1 + 2.0 * m) * jn(1 + 2 * m, v)
        UU_2 += (-1.0) ** m * (u / v) ** (2 + 2.0 * m) * jn(2 + 2 * m, v)
    return UU_1, UU_2


def pystar_circle(
    wl, dist_xo_ss, dist_ss_t, R_ss, N_x=8001, N_t=257, dx=0.0075, dt=0.01875
):
...
@pytest.mark.parametrize(
    "wl, dist_xo_ss, dist_ss_t, R_ss, N_x, N_t, dx, dt", test_data
)
def test_circle(wl, dist_xo_ss, dist_ss_t, R_ss, N_x, N_t, dx, dt):
    field_circ = pystar_circle(
        wl, dist_xo_ss, dist_ss_t, R_ss, N_x=N_x, N_t=N_t, dx=dx, dt=dt
    )
    analytic_circ = calculate_circle_solution(
        wl, dist_xo_ss, dist_ss_t, R_ss, N_t=N_t, dt=dt
    )
    assert (
        np.max(np.abs(np.abs(field_circ) - np.abs(analytic_circ))) ** 2 < 1e-6
    )
